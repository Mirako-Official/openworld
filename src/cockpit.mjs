...
export function createCockpit(){
  ...
  return {update(vehicle,enabled,keys,paused,dt,walking=true){
    ...
    if(car){summary.textContent+=' · '+(vehicle.autodriveStatus||'R 自动驾驶');handbrakeHelp.innerHTML='<kbd>Shift</kbd> 手刹 · <kbd>R</kbd> 自动驾驶开关';updateInstruments(vehicle,state,paused?'已暂停':handbrake?'手刹':state.braking?'制动中':state.speed?'行驶中':'就绪');}
  }};
}
